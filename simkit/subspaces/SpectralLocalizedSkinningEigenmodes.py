...
class SpectralLocalizedSkinningEigenmodesParams(SubspaceParams):
    def __init__(self, d, m, k,c, cache_dir = None, read_cache=False, sparse=False, threshold=1e-3):
        self.d = d 
        self.m = m  
        self.k = k
        self.c = c
...
class SpectralLocalizedSkinningEigenmode(Subspace):

    def __init__(self, X, T, params: SpectralLocalizedSkinningEigenmodesParams):
        ...
        if params.read_cache:
            try:
                [ W, B, cI, cW, labels, Wd, cId, conI, SB, slabels] = self.read_subspace(cache_dir)
                well_read = True
            except:
                warnings.warn("Warning : Couldn't read subspace from cache. Recomputing from scratch...")
        if not well_read:
            [ W, B, cI, cW, labels, Wd, cId, conI, SB, slabels] = self.compute_subspace(X, T, params)
            if params.cache_dir is not None:
                os.makedirs(cache_dir, exist_ok=True)
                self.save_subspace(cache_dir, W,  B, cI, cW, labels, Wd, cId, conI, SB, slabels)
        self.W = W
        self.B = B
        self.cI = cI
        self.cW = cW
    ...
    def read_subspace(self, cache_dir):
        ...
        slabels = np.load(cache_dir + "slabels.npy")
        return W, B, cI, cW, labels, Wd, cId, conI, SB, slabels
    
    def save_subspace(self,cache_dir,  W,  B, cI, cW, labels, Wd, cId, conI, SB, slabels):
