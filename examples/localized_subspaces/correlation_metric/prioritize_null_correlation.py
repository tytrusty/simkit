...
cor[np.abs(cor) > threshold] = 0
for m in ms:
    [E, G] = eigs(L, M=M, k = m)
    sub_cor_eig  = subspace_corrolation(Hi,  B=G, M=M)
    sub_cor_eig[np.abs(sub_cor_eig) > threshold] = 0
    ...
    for c, h in enumerate(hs):
        ...
        sub_cor_riv[np.abs(sub_cor_riv) > threshold] = 0
...
plt.savefig(os.path.join(dir, 'correlation_error_ablate_h_prioritize_null_threshold_' + str(threshold) + '.png'))
plt.show()
# plt.show()